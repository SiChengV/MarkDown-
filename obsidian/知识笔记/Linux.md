...
修改进程亲核性：`taskset -cp 0-3 {pid}`  命令修改进程亲核性：将进程绑定到0-3核心
...
| 字段                    | 含义                       |
| --------------------- | ------------------------ |
| rchar                 | 用户态读了多少字节（read 系统调用参数加总） |
| wchar                 | 用户态写了多少字节（write 参数加总）    |
| syscr                 | read 系统调用次数              |
| syscw                 | write 系统调用次数             |
| read_bytes            | 实际从磁盘读取的字节数（包括 mmap 触发的） |
...
<font size=5>**CGROUP目录**</font>

子系统下的每个目录代表一个层级，层级内文件如下：

* tasks：加入这个层级的进/线程id（往该文件添加进/线程号时仅添加那一个线程到该组）
* cgroup.procs：加入这个层级的线程组id（不可往该文件中添加线程，往该文件添加进程号时会将所有子线程都一并加入该组）
* notify_on_release：
* release_agent：（只存在于top层级）

<font size=5>**cpu子系统**</font>

* cpu.cfs_quota_us和cpu.cfs_period_us配合使用可限制cpu的绝对使用上限 （这两个文件仅对非实时调度策略的进程生效）

<font size=5>**cgroups使用方法**</font>

cgroups通过VFS把相关功能暴露给用户
...
* 查看进程所属cgroup组：`cat /proc/{pid}/cgroup`
* cgroup组的配置信息所在目录：`/sys/fs/cgroup/`
* 查看A组的CPU限额： `/sys/fs/cgroup/cpu/A/cpu.cfs_quota_us`  除以 `/sys/fs/cgroup/cpu/A/cpu.cfs_period_us` = 该组的cpu核数限制

### SELINUX机制

修改文件标签命令：`chcon system_u:object_r:app_t:s0 {文件名}`

查看当前selinux状态 `sestatus`

`sesearch -s unconfined_t -t app_t -A`      查看unconfined_t对app_t的访问权限

以某个标签起一个进程：`runcon -t  xxx_t 二进制`

* selinux策略规则
  
  `type_transition unconfined_t app_t:process app_t;`
  
  当一个 **`unconfined_t` 类型的进程**（也就是你的父进程）执行一个 **`log_daemon_t` 类型的可执行文件** 时，**自动将新创建的子进程的类型设置为 `log_daemon_t`**。(通过这个规则确保被拉起的进程的selinux标签正确)

#### selinux工具

* restorecon 恢复文件安全上下文
* ausearch 检查audit日志    eg：`ausearch -m avc -p 1901` 代表返回1901号进程的权限受限日志
...
使用` (?<!\S)-Werror(?!\S)`即可在`asdf -Werror -Werror=ASDasdf -Werrorsd`中全字匹配出-Werror
...
分为标准信号和实时信号，在处理期间受到标准信号会丢弃。收到实时信号会保存，后续按顺序执行。
...
target_compile_options(your_target PRIVATE -std=c++17)
target_compile_features(your_target PRIVATE -std=c++17)    # 跨平台可移植性更好，但是像-Werror之类的编译参数不能使用
...
关闭soket需要先使用shutdown函数通知对端，然后再调用close函数释放文件描述符
...
* 使用方法
  ...
  - CPUs utilized ： task-clock毫秒/time elapsed秒 *1000 / 100 (CPU的占用率)
...
* 抓取缺页场景
  ...
  `perf record -g -e page-faults -p <pid> -- sleep 120`   捕获pid对应进程的缺页数据，包括major-faults（只记录主缺页（磁盘 IO 导致））和minor-faults（只记录次缺页（在内存但无映射））    加上-g代表记录调用栈信息
...
如果要检测一个会创建子进程的程序，需要加--trace-children=yes来将子进程纳入监控。注意打上符号表。

**适用程序运行时的内存增长，如运行时内存泄露**

```shell
valgrind --tool=massif --log-file="valgrind.log" ./your_app
ms_print {上述命令产生的文件}      # 解析文件并打屏
```

使用--tool=massif 可以用来分析内存增长过程&峰值来源，它可以抓取多个内存快照，通过分析多个内存快照，以及峰值内存快照的差别，可以看出是哪一处的调用占用了大量内存。

### strace工具

抓取进程的函数调用情况

`strace -k -e ioctl -p xxx`

* -k:  抓取调用栈信息
* -e：要抓取的系统调用
* -p：抓取的线程号
* -f：配合-p {pid} 使用，抓取进程的所有线程
* -c：统计调用命中次数，输出总结。和 -f 互斥
...
```shell
cd /sys/kernel/debug/tracing/     # 进入工作目录
echo 'p:auto ext4_file_write_iter comm=$comm' > kprobe_events    # 定义Kprobe探测点
echo 1 > events/kprobes/auto/enable         # 激活探测器
cat trace_pipe                            # 实时查看输出

echo 0 > events/kprobes/auto/enable     # 关闭探测器
echo -:auto >> kprobe_events               # 删除探测点
```

**`p`**：表示这是一个 **kprobe**（内核动态探测点），它可以在内核代码执行的任意位置打桩。

**`:auto`**：这是你给这个探测点起的组名/事件名。

**`ext4_file_write_iter`**：这是你想要监控的内核函数名。它是 EXT4 文件系统处理文件写入的核心入口。

**`comm=$comm`**：这是一个参数采集指令。它告诉内核：“当函数被触发时，记录下当前进程的名字（`$comm`）并赋值给变量 `comm`”。
...
使用信号处理函数自定义收到信号的处理行为

```c++
struct sigaction sa;
sa.sa_handler = &SigHandler;  // 注册信号处理函数
sa.sa_flags = 0;    // 默认情况下，进程收到信号会唤醒被阻塞的系统调用，并设置error为EINTR，如果不希望唤醒被阻塞的系统调用，可将sa_flags设置为SA_RESTART
sigemptyset(&sa.sa_mask);    // 清空信号集
sigaction(SIGINT, &sa, nullptr);  // 注册信号函数
```
...
* ps
  ...
  `ps -afx` 查看进程启动顺序以及拉起关系

* top
  
  `top -H -p ${pid}` 查看这个进程的所有线程占用cpu大小
  
  `top -d 1 -p ${pid}` 修改刷新频率为1秒刷新一次
...
`sudo tcpdump -s 0 -i ethg0 dst host 192.168.10.6 and dst port 50512  -w /root/dump.pcap`

* -s 0 表示抓取完整数据包
* -i ethg0 dst host 192.168.10.6 and dst port 50512 代表抓取ethg0网卡指定ip地址和端口的数据，dst指该包的目的地

#### 调试相关

`sudo sysctl kernel.kptr_restrict=0` 允许用户访问内核符号地址。例如使用perf抓取数据时，内核相关的调用会显示为未知，这时使用此命令后再去抓取perf数据，就可以正常显示内核相关符号。
...
* RGID/RUID: 真实组ID、用户ID
  
  即文件原本所属的id

* EGID/EUID：有效组ID、用户ID
  
  即文件被执行时，是以什么ID去执行的。比如通过setgid、setuid修改之后，实际修改的是有效ID。
