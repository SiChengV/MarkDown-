...
子类默认不会继承基类的带参构造函数。要继承的话可以使用`using BaseClass::BaseClass`
...
作为基类的析构函数必须为virtual的，不让在使用多态特性时，仅会调用基类的析构函数，而使用虚析构函数的场景，会先调用子类的析构函数，再调用基类的析构函数
...
* struct tm 可读时间结构
  ...
  定义在<time.h>中，不是人类可读时间（年月日时分秒等），适合显示时间、格式化输出。常与 `localtime()`, `gmtime()`, `strftime()` 等一起使用

* struct timespec 高精度时间结构
  
  定义在<time.h>中，表示高精度时间戳，纳秒级别。使用clock_gettime接口获取
...
* 左值
  ...
  代表可以被取地址的表达式。左值引用：int&
  
  std::ref(x)：返回一个可拷贝、赋值的对象，但内部保存着对x的引用。主要用在需要把“引用”包装成一个可拷贝对象的场景。
  ...
  int main() {
      int n = 5;
      auto f = std::bind(print, std::ref(n));  // n 按引用绑定。如果不使用std::ref则会先拷贝一份n，然后作为引用传递给print。
      n = 10;
      f();  // 输出 10
  }
  ```
...
* 模板中
  ...
  在模板函数中，同样的 `T&&` 入参将不再代表右值，转而代表转发引用，一般与完美转发搭配使用
...
// regex_match对整条字符串进行 匹配，不符合则返回false
```
...
<font size=5>**std::promise**</font>

生产者端

<font size=5>**std::future**</font>

消费者端

**通过wait或wait_for接口阻塞等待结果，返回值**（`std::future_status` 枚举）：

1. `std::future_status::ready` —— 任务已完成，结果可获取。
2. `std::future_status::timeout` —— 等待超时，任务还没完成。
3. `std::future_status::deferred` —— 任务是 *deferred* 模式（延迟执行，直到调用 `get()` 才会真正运行）。

通过**get**接口获取异步执行结果

### std::packaged_task

和std::funciton一样内部存储可调用对象，但是会将结果放到一个std::future里，供后续获取
...
* compare_exchange_weak
  ...
  一般在while循环中使用，允许虚假失败（实际结果应该为true并执行赋值，但返回了false）
...
std::bitset<16> bar (std::string("0101011111"));
...
sort函数使用自定义比较器的场景，需要保证比较函数是严格弱序的，否则会产生未定义行为，也即比较函数应该满足：

**不可自反性**：`comp(a, a)` 必须返回 false

**传递性**：如果 `comp(a, b)` 且 `comp(b, c)`，则必须 `comp(a, c)`

**反对称性**：如果 `comp(a, b)` 为 true，那么 `comp(b, a)` 必须为 false

**等价关系的一致性**：如果 `comp(a, b)` 和 `comp(b, a)` 都为 false，则认为 `a` 和 `b` 是“等价的”，且 `comp(a, c)` 与 `comp(b, c)` 的结果应该一致

## ASAN地址消毒器应用

在cmakelists.txt中加入一句即可：

`set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fsanitize=address -fno-omit-frame-pointer -fno-stack-protector -fsanitize-recover=address -O0")`

| 编译选项                       | 说明                                                  | 开始支持版本          |
| -------------------------- | --------------------------------------------------- | --------------- |
| -fsanitize=address         | 开启内存越界检查                                            | gcc 4.8         |
| -fsanitize=leak            | 开启内存泄露检查                                            | gcc 4.9(仅64位支持) |
| -fsanitize-recover=address | 一般后台程序为保证稳定性，不能遇到错误就简单退出，而是继续运行，采用该选项支持内存出错之后程序继续运行 | gcc 6.1         |
| -fno-stack-protector       | 去使能栈溢出保护                                            | gcc 4.8         |
| -fno-omit-frame-pointer    | 去使能栈溢出保护以得到更容易理解的stack trace                        | gcc 4.8         |
...
| log_path=/opt/asan.log          | 内存检查问题日志存放文件路径      |
| detect_leaks=1                  | 使能内存泄露检测            |
| detect_stack_use_after_return=1 | 检查访问指向已被释放的栈空间      |

eg：export ASAN_OPTIONS="halt_on_error=0: log_path=/opt/huawei/asan_nlb.log"

```
detect_container_overflow=0   关闭这种错误的检测
```
...
### 释放后使用问题（use after free）

背景：服务端处理外部请求场景，假设外部请求进来时刚判断完服务端的状态，判断通过后准备继续往后触发并执行任务，此时服务端退出开始走析构流程，这种场景就有可能发生任务正在执行，但是服务端析构导致coredump。该场景本质是析构流程不合理导致，最好的解决方法应该是梳理正确析构流程。

解决方法：对外接口中使用一个RAII思想的接口，在对外接口调用完成后析构。进程在析构流程中判断是否有任务正在执行，有的话则延迟析构，知道所有任务处理完成。
...
// SafeCall保证pm析构后不响应外部调用，外部调用进行中pm不析构
class SafeCall {
public:
    SafeCall(APP &node)          // APP代表要实现上述功能的主体，一般是负责管理主体流程的对象
        : node_(node) {
        if (node_.state_.load() != State::INITED) {
            return;
        }
        node_.activeWork_.fetch_add(1);

        // 再次检查是否还在运行状态
        if (node_.state_.load() != State::INITED) {
            node_.activeWork_.fetch_sub(1);
            node_.exitCv_.notify_all();
            return;
        }
...
class APP {       // 此类也可作为基类，继承此类的子类都可以使用上述机制
// APP自身数据
// ...
public:
    void OutFun() {     // 对外接口
    ...
    void Stop() {    // 析构是调用stop
        ...
        if (!state_.compare_exchange_strong(expected, State::DEINITED)) {
            return;             // 已析构，直接返回
        }
        std::unique_lock<std::mutex> lk {exitMtx_};
        exitCv_.wait(lk, [this] {    // 等待对外接口调用完成，触发条件变量
            bool ret {activeWork_ == 0U};          // 检查是否还要其他接口正在调用
            if (!ret) {
                PM_LOG_WARN << "Task is running when OnTerminate.";
            }
            return ret;
        });
        // 实际析构业务处理 
...
private:
    std::atomic<State> state_ {State::UNINIT};    // 服务端状态
    std::atomic<size_t> activeWork_ {0U};        // 正在运行的任务数量
    std::mutex exitMtx_;
